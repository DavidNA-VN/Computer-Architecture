...
.text 
main: 
	la      t0, handler             #địa chỉ của chương trình con xử lí ngắt
	csrrs   zero, utvec, t0 	# thiết lập địa chỉ ISR, địa chỉ ngắt
...
handler: 
	# Saves the context 
	addi    sp, sp, -16 		#dịch con trỏ ngăn xếp để chừa chỗ lưu 4 thanh ghi
	sw      a0, 0(sp) 
	sw      a1, 4(sp) 		#lưu các giá trị thanh ghi tạm vì ISR có thể thay đổi
	sw      a2, 8(sp) 
	sw      a7, 12(sp) 
     
	# Handles the interrupt
	csrr    a1, ucause	#lấy giá trị thanh ghi ucause
	li      a2, 0x7FFFFFFF 
	and     a1, a1, a2      # Clear interrupt bit to get the value 

	li      a2, MASK_CAUSE_KEYPAD 
	beq     a1, a2, keypad_isr 	#nếu là ngắt từ keypad thì gọi keypad-isr
	j       end_process 
...
new:
	li s6, 0    # reset toán hạng repeat khi nhập số mới
	li s7, 0    # reset toán tử repeat
...
phep_tinh:
	...
	li s5, 0			# reset s5-cờ dấu bằng khi bắt đầu phép tính mới
...
update_op:
	...
	li a7, 11
	li s6, 0    # reset toán hạng repeat khi nhập phép toán mới
	li s7, 0    # reset toán tử repeat
...
#---------------------------------------------------------------------------------------
.text
# ... giữ nguyên phần code trước đó ...

phep_bang:
    li s5, 1                # vừa mới nhấn '='

    bnez s7, repeat_operation     # nếu đã có phép toán repeat thì lặp lại phép toán
...
cong:
    add s0, s0, s1
    mv s6, s1               # lưu toán hạng repeat
    mv s7, a4               # lưu toán tử repeat
    li a7, 1
    mv a0, s1
    ecall
    li s4, 0
...
repeat_operation:
    li t3, 10
    beq s7, t3, repeat_add

    li t3, 11
    beq s7, t3, repeat_sub

    li t3, 12
    beq s7, t3, repeat_mul

    li t3, 13
    beq s7, t3, repeat_div

    li t3, 14
    beq s7, t3, repeat_rem
...
hienthi:
	...
	bge s3, zero, hienthisoduong	#s3>=0 thì hiển thị bình thường số dương
...
hienthisoduong:	
	...
	div t5, s3, t3			# chia cho 10 để lấy hàng chục
    	add t5, t5, a3       		# Địa chỉ của A[t5]
    	lb a0, 0(t5)         		# set value for segments
    	jal SHOW_7SEG_LEFT  		# show
	
	rem t6, s3, t3			# chia dư cho 10 để lấy hàng đơn vị 
...
reset_all:
	...
	li s6, 0    # reset toán hạng repeat
	li s7, 0    # reset toán tử repeat
...
end_process: 
	lw      a7, 12(sp) 	# trong handler đã lưu các thanh ghi a0,1,2,7 để tránh mất dữ liệu
	lw      a2, 8(sp) 
	lw      a1, 4(sp) 
	lw      a0, 0(sp) 
	addi    sp, sp, 16 	# cộng lại con trỏ ngăn xếp
